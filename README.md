...
기존에는 담당자가 나라장터 웹사이트에 직접 접속해 수동으로 공고를 조회하고 정리했습니다. 매일 반복 조회 작업이 필요했고, 누락이나 중복 발생 가능성도 있었습니다.
...
저장 중 프로세스 중단 시 불완전한 파일이 남는 문제를 방지. 임시 파일에 완전히 쓴 뒤 `os.replace()`로 원자적 교체, 실패 시 임시 파일 자동 삭제.
...
### 설정값 외부화
API 키, 네트워크 경로 등 민감한 설정을 `.env`로 분리, `frozen=True` 데이터클래스로 불변 설정 객체 구성.
...
전체 리팩토링 과정 - [리팩토링 회고](./리팩토링/리팩토링%20회고.md)
