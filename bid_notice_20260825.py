...
class Config:
    ...
    teams_webhook_url: str = field(
        default_factory=lambda: os.environ["TEAMS_WEBHOOK_URL"]
    )
...
# 수동 날짜 실행 시
#CFG = Config(manual_start="202607160000", manual_end="202607162359")
...
def setup_logging(log_root: str) -> None:
    now = now_kst()
    log_dir   = Path(log_root) / now.strftime("%Y%m")
...
def fetch_all_pages(session: requests.Session, url: str, base_params: dict) -> tuple[list[ET.Element], list[int]]:
    """
    반환: (수집된 item 리스트, 최종 실패한 페이지 번호 리스트)
    """
    all_items: list[ET.Element] = []
    failed_pages: list[int] = []
    total_pages = 0
    
    def fetch_one_page(page: int, max_retry: int = 3) -> ET.Element | None:
        for attempt in range(1, max_retry + 1):
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f" [{page}페이지] XML 파싱 실패"
                        f"(시도 {attempt}/{max_retry}, status={resp.status_code}, "
                        f"content-type={resp.headers.get('Content-Type')}) "
                        f"body={resp.content[:300]!r}"
                    )
                    
            except requests.exceptions.RequestException as e:
                logger.warning(f"   [{page}페이지] 요청 실패(시도 {attempt}/{max_retry}): {e}")
            
            if attempt < max_retry:
                time.sleep( 2 ** attempt)
        
        logger.error(f" [{page}페이지] 최종 실패 ({max_retry}회 시도 완료)")
        return None
    
    # 1페이지
    root = fetch_one_page(1)
    if root is None:
        raise RuntimeError("1페이지 조회 실패 - totalCount 확보 불가")
...
def _filter_by_org_and_type(items: list[ET.Element], org_field: str, type_field: str) -> list[ET.Element]:
    """
...
def _filter_by_org(items: list[ET.Element], org_field: str) -> list[ET.Element]:
    """
    발주계획용 org 필터
    """
    return [
...
def write_meta(ws: Worksheet, cell: str, label: str, value: str) -> None:
    """
...
def apply_excel_style(ws: Worksheet, start_row: int, column_widths: dict, number_cols: list | None = None) -> None:
...
def write_no_data(ws: Worksheet, start_row: int, merge_cols: int = 5) -> None:
    """데이터 없을 때 안내 문구 삽입"""
    no_data_row = start_row + 2
    
    ws.merge_cells(start_row=no_data_row, start_column=1, end_row=no_data_row, end_column=merge_cols)
...
def notify_teams(title: str, msg: str) -> None:
    ...
    try:
        ...
        response = requests.post(CFG.teams_webhook_url, json=payload, timeout=5,)
        
        logger.info(
            f"Teams 응답: status={response.status_code}, "
        ...
        logger.warning(f"Teams 알림 전송 실패: {e}")
...
def fetch_입찰공고(session, start_date: str, end_date: str) -> tuple[pd.DataFrame, list[int]]:
    ...
    items, failed_pages = fetch_all_pages(session, URL_BID, base_params)
    
    if failed_pages:
        logger.error(f"입찰공고 조회 중 실패한 페이지: {failed_pages}")
...
def save_입찰공고(df: pd.DataFrame, failed_pages: list[int] | None,
                start_date: str, end_date: str, run_time: str
                ) -> str | None:
    ...
    with atomic_write(file_path) as tmp:
        with pd.ExcelWriter(tmp, engine="openpyxl") as writer:
            ...
            if failed_pages:
                write_meta(ws, "A5", "⚠ 주의", f"조회 실패 페이지 {failed_pages}")
                start_row += 1
...
def fetch_사전규격(session, start_date: str, end_date: str) -> tuple[pd.DataFrame, list[int]]:
    logger.info("\n===== 02. 사전규격공개 조회 시작 =====")
    ...
    items, failed_pages = fetch_all_pages(session, URL_SPEC, base_params)

    if failed_pages:
        logger.error(f"사전규격 조회 중 실패한 페이지: {failed_pages}")
...
def save_사전규격(df: pd.DataFrame, failed_pages: list[int] | None,
                start_date: str, end_date: str, run_time: str
                ) -> str | None:
    ts        = now_kst().strftime("%Y%m%d_%H%M")
    ...
    with atomic_write(file_path) as tmp:
        with pd.ExcelWriter(tmp, engine="openpyxl") as writer:
            ...
            if failed_pages:
                write_meta(ws, "A8", "⚠ 주의", f"조회 실패 페이지 {failed_pages}")
                start_row += 1
...
def fetch_발주계획(session, start_date: str, end_date: str,
                  order_start_ym: str, order_end_ym: str) -> tuple[pd.DataFrame, list[int]]:
    logger.info("\n===== 03. 발주계획 조회 시작 =====")
    ...
    items, failed_pages = fetch_all_pages(session, URL_PLAN, base_params)

    if failed_pages:
        logger.error(f"발주계획 조회 중 실패한 페이지: {failed_pages}")
...
def save_발주계획(df: pd.DataFrame, failed_pages: list[int] | None, 
                start_date: str, end_date: str, run_time: str
                ) -> str | None:
    ts        = now_kst().strftime("%Y%m%d_%H%M")
    ...
    with atomic_write(file_path) as tmp:
        with pd.ExcelWriter(tmp, engine="openpyxl") as writer:
            ...
            if failed_pages:
                write_meta(ws, "A9", "⚠ 주의", f"조회 실패 페이지 {failed_pages}")
                start_row += 1
...
def run_all_fetches(
    ...
    """
    세 API 순서대로 호출하고 (DataFrame, 실패 페이지 목록) 딕셔너리로 반환
    """
    df_bid, fail_bid = fetch_입찰공고(session, start_date, end_date)
    df_spec, fail_spec = fetch_사전규격(session, start_date, end_date)
    df_plan, fail_plan = fetch_발주계획(session, start_date, end_date, order_start_ym, order_end_ym)
    return {
        "입찰공고": (df_bid, fail_bid),
        "사전규격": (df_spec, fail_spec),
        "발주계획": (df_plan, fail_plan),
    }

def save_all(
    results: dict[str, tuple[pd.DataFrame, list[int]]],
    start_date: str,
    end_date: str,
    run_time: str,
) -> dict[str, str | None]:
    """
    (DataFrame, 실패 페이지 목록) 딕셔너리를 받아 엑셀 저장 후 경로 딕셔너리 반환
    """
    return {
...
def log_summary(results: dict[str, tuple[pd.DataFrame, list[int]]], paths: dict[str, str | None]
    ...
    for key, (df, failed_pages) in results.items():
        path = paths.get(key) or "저장 없음"
        logger.info(f"  {key} : {len(df)}건  →  {path}" + 
                    (f"  ⚠ 실패페이지 {failed_pages}" if failed_pages else ""))
        if failed_pages:
            all_failed.append((key, failed_pages))
    return all_failed
...
def run() -> None:
    try:
        ...
        all_failed = log_summary(results, paths)
        
        if all_failed:
            msg = "\n".join(
                f"- {key}: 실패 페이지 {page}" for key, pages in all_failed
            )
